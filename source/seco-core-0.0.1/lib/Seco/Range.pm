...
use constant NODE_CLUSTER => '/home/tops/candy/whoismycluster/node_cluster.dat';
...
@EXPORT_OK = qw/expand_range get_cluster_nodes range_set_altpath
    compress_range sorted_expand_range nodes_parser/;
%EXPORT_TAGS = (
    all => [@EXPORT_OK],
...
sub _get_group {
    ...
    my $group_fmt = "$node_fmt-\%d";
    my $sum = $digits + $count;
    my $pos = 0;
    if (length($sum) == length($digits)) {
	for (0..length($sum) - 1) {
	    $pos = $_;
	    last if substr($digits, $_, 1) ne substr($sum, $_, 1);
	}
    }
...
sub expand_range {
	...
	s/@([A-Z][^,]+)/%GROUPS:$1/g;  # shorthand notation for groups
	s/@([a-z][^,]+)/%HOSTS:$1/g;   # shorthand notation for hosts
    }
	...
	if (substr($range,0,1) eq "(") {
	    ...
	    @all = ( hostname() ) unless @all;
	    my @nodes = grep { /$regex/ } @all;
	    @nodes{@nodes} = undef;
	} else {
	    # simple range
	    if ($range =~ /^
...
=head2 SIMPLE RANGES
    ...
    foo1-2.search.scd.taobaocom ==
	foo1.search.scd.taobao.com-foo2.search.scd.taobao.com # domain support
