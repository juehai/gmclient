...
use strict;
use Seco::Gemstone::YAML::Node;
use Seco::Gemstone::YAML::Transfer;
use Carp;
...
sub new {
    ...
    my $o = {
             ...
             level => 0,
             anchor => 1,
             Indent => $Seco::Gemstone::YAML::Indent,
             UseHeader => $Seco::Gemstone::YAML::UseHeader,
             UseVersion => $Seco::Gemstone::YAML::UseVersion,
             SortKeys => $Seco::Gemstone::YAML::SortKeys,
             AnchorPrefix => $Seco::Gemstone::YAML::AnchorPrefix,
             DumpCode => $Seco::Gemstone::YAML::DumpCode,
             LoadCode => $Seco::Gemstone::YAML::LoadCode,
             ForceBlock => $Seco::Gemstone::YAML::ForceBlock,
             UseBlock => $Seco::Gemstone::YAML::UseBlock,
             UseFold => $Seco::Gemstone::YAML::UseFold,
             CompressSeries => $Seco::Gemstone::YAML::CompressSeries,
             InlineSeries => $Seco::Gemstone::YAML::InlineSeries,
             UseAliases => $Seco::Gemstone::YAML::UseAliases,
             Purity => $Seco::Gemstone::YAML::Purity,
             DateClass => $Seco::Gemstone::YAML::DateClass,
            };
    bless $o, $class;
    set_default($o, 'DumpCode', $Seco::Gemstone::YAML::UseCode);
    set_default($o, 'LoadCode', $Seco::Gemstone::YAML::UseCode);
    return $o if is_valid($o);
}

my $global = {}; # A global lookup
sub Bless { Seco::Gemstone::YAML::bless($global, @_) }
sub Blessed { Seco::Gemstone::YAML::blessed($global, @_) }

sub blessed {
    my ($o, $ref) = @_;
    $ref = \$_[0] unless ref $ref;
    my (undef, undef, $node_id) = Seco::Gemstone::YAML::Node::info($ref);
    $o->{blessed}{$node_id};
}
...
sub bless {
    ...
    if (not defined $blessing) {
        $ynode = Seco::Gemstone::YAML::Node->new($ref);
    }
    elsif (ref $blessing) {
...
# OO version of Dump. Seco::Gemstone::YAML->new->dump($foo); 
sub dump {
    $o = shift; 
...
sub _prewalk {
    my $value;
    my ($class, $type, $node_id) = Seco::Gemstone::YAML::Node::info(\$_[0]);
    # Handle typeglobs
    if ($type eq 'GLOB') {
        $value = $o->{transferred}{$node_id} = 
          Seco::Gemstone::YAML::Transfer::glob::yaml_dump($_[0]);
        return _prewalk($value);
    }
    # Handle regexps
    if (ref($_[0]) eq 'Regexp') {  
        $o->{transferred}{$node_id} = Seco::Gemstone::YAML::Transfer::regexp::yaml_dump($_[0]);
        return;
    }
    ...
    if ($type eq 'CODE') {
        ...
        Seco::Gemstone::YAML::Transfer::code::yaml_dump($o->{DumpCode},
                                        $_[0], 
    ...
    # Handle blessed things
        ...
        if ($class eq $o->{DateClass}) {
            ...
            Seco::Gemstone::YAML::Transfer::blessed::yaml_dump
            ...
            $value = Seco::Gemstone::YAML::Transfer::blessed::yaml_dump($value);
        }
        $o->{transferred}{$node_id} = $value;
        (undef, $type, $node_id) = Seco::Gemstone::YAML::Node::info($value);
    }
    ...
    if ($type eq 'REF' or $type eq 'SCALAR') {
        $value = Seco::Gemstone::YAML::Transfer::ref::yaml_dump($value);
        $o->{transferred}{$node_id} = $value;
        (undef, $type, $node_id) = Seco::Gemstone::YAML::Node::info($value);
    }
    # Handle ref-to-glob's
        ...
        my $ref_ynode = $o->{transferred}{$node_id} =
          Seco::Gemstone::YAML::Transfer::ref::yaml_dump($value);

        my $glob_ynode = $ref_ynode->{&VALUE} = 
          Seco::Gemstone::YAML::Transfer::glob::yaml_dump($$value);

        (undef, undef, $node_id) = Seco::Gemstone::YAML::Node::info($glob_ynode);
...
sub _emit_node {
    ...
    if ($ref and $ref ne 'Regexp') {
        (undef, $type, $node_id) = Seco::Gemstone::YAML::Node::info($_[0]);
    }
    else {
        $type = $ref || 'SCALAR';
        (undef, undef, $node_id) = Seco::Gemstone::YAML::Node::info(\$_[0]);
    }
...
sub _parse_explicit {
    ...
    if ($explicit =~ m{^(int|float|bool|date|time|datetime|binary)$}) {
        my $handler = "Seco::Gemstone::YAML::_load_$1";
        no strict 'refs';
        return &$handler($node);
    }
    elsif ($explicit =~ m{^perl/(glob|regexp|code|ref)\:(\w(\w|\:\:)*)?$}) {
        my ($type, $class) = (($1 || ''), ($2 || ''));
        my $handler = "Seco::Gemstone::YAML::_load_perl_$type";
        no strict 'refs';
        if (defined &$handler) {
        ...
        no strict 'refs';
        if (defined &$handler) {
            return &$handler(Seco::Gemstone::YAML::Node->new($node, $explicit));
        }
        else {
        ...
        return Seco::Gemstone::YAML::Node->new($node, $explicit);
    }
}
...
sub AUTOLOAD {
    (my $autoload = $Seco::Gemstone::YAML::AUTOLOAD) =~ s/^Seco::Gemstone::YAML:://;
    if ($autoload =~ /^[A-Z]/ and 
        ref($_[0]) eq 'Seco::Gemstone::YAML' and
    ...
    croak "Can't autoload '$Seco::Gemstone::YAML::AUTOLOAD'\n"
      unless $autoload =~ /^YAML_(PARSE|LOAD|DUMP|EMIT)_(ERR|WARN|USAGE)/;
    require Seco::Gemstone::YAML::Error;  
    $o->{error} = Seco::Gemstone::YAML::Error->new($autoload, $o->{line}, $o->{document}, @_);
    my $o_save = $o;
