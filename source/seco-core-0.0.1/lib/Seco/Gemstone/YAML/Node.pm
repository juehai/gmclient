package Seco::Gemstone::YAML::Node;
...
sub new {
    ...
    $self->{KIND} = (not defined $type) ? 'scalar' :
		    ...
		    croak "Can't create Seco::Gemstone::YAML::Node from '$type'";
    family($self, ($family || ''));
...
sub family {
    my ($self, $value) = @_;
    if (defined $value) {
       	$self->{FAMILY} = Seco::Gemstone::YAML::Family->new($value);
	return $self;
    }
    else {
       return $self->{FAMILY};
...
#==============================================================================
package yaml_scalar;
@yaml_scalar::ISA = qw(Seco::Gemstone::YAML::Node);
...
@yaml_sequence::ISA = qw(Seco::Gemstone::YAML::Node);
...
@yaml_mapping::ISA = qw(Seco::Gemstone::YAML::Node);
...
Seco::Gemstone::YAML::Node - A generic data node that encapsulates YAML information

=head1 SYNOPSIS

    use Seco::Gemstone::YAML;
    use Seco::Gemstone::YAML::Node;
    
    my $ynode = Seco::Gemstone::YAML::Node->new({}, 'ingerson.com/fruit');
